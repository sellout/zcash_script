...
pub use interpreter::{
    CallbackTransactionSignatureChecker, DefaultStepEvaluator, HashType, ScriptError,
    SighashCalculator, SignedOutputs, VerificationFlags,
};
...
pub fn normalize_error(err: Error) -> Error {
    match err {
        Error::Ok(serr) => Error::Ok(match serr {
            ScriptError::Interpreter(ie) => match ie {
                ...
                interpreter::Error::ScriptNum(_) => ScriptError::UnknownError,
            ...
            ScriptError::Read(_) => interpreter::Error::BadOpcode(None).into(),
            _ => serr,
        }),
        _ => err,
    }
